...
import { ACCESS_TOKEN, AuthState, AUTH_USER, LoginResponse, RegisterResponse, SearchUsersResponse, SEARCH_USERS_QUERY, User, UserResponse, UsersResponse } from 'src/app/shared';
...
export class AuthService {
  ...
  constructor(
    ...
    this.isLoggedInAsync$ = this.authState$.pipe(
  ...
  searchUsers(searchQuery: string, offset: number, limit: number): SearchUsersResponse {
    ...
    return {
      data: feedQuery.valueChanges.pipe(
        map(result => result.data)
      ),
      fetchMore: fetchMore
    }
